class DvTasks {
    ...
    relDateString(d) {
      if (!(d instanceof dv.luxon.DateTime)) return '–'
    ...
    getTodayTasks(args) {
      const { dv, date, that } = args
      const finalDate = date ?? dv.current().file.name
      return this.getTasksTable({
        ...args,
        filterFn: t => t.status != 'Completed' && t.dueDate && t.dueDate?.hasSame(dv.luxon.DateTime.fromISO(finalDate), 'day')
      })
    }
    ...
    getOverdueTasks(args) {
      ...
      return this.getTasksTable({
        ...args,
        prependText: 'Overdue',
        filterFn: t => t.dueDate && t.dueDate < dv.luxon.DateTime.fromISO(finalDate) && t.status != 'Completed'
      })
    }
    ...
    getTasksTable(args) {
      ...
      const pages = dv.pages("#test").sort(getSortProp, sortOrder).where(filterFn)
      if (pages.length === 0) {
