...
module.exports = async function listFiles(params) {
    ...
    // https://stackoverflow.com/questions/29901266/regular-expression-to-find-last-folder-in-a-path
    // _PARA/Projects/DCR Farm ---/Scratchpad.md
    let regex = /.*\/([^\/]+)\//;
    chosenFilePath = notesDisplay.path;
    let chosenProject = chosenFilePath.match(regex)[1]; // "DCR Farm ---"

    // Pass selected note's path to notes variable
    params.variables = { notes: notesDisplay.path, projectName: chosenProject };
}
